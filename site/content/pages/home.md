---
title: Home
seo_description: >-
  Latitude 26 brings unhurried, expert medical care directly to your home,
  office, or yacht across Miami, Fort Lauderdale, and Palm Beach — no
  insurance, no waiting, no compromise.
hero:
  headline: Private Medicine. Personal Care. Prioritized for You.
  intro: >-
    South Florida's most discerning individuals and families deserve more than
    a rushed office visit and a crowded waiting room. Latitude 26 brings
    unhurried, expert medical care directly to your home, office, or yacht —
    with no insurance, no waiting, and no compromise.
  actions:
    - label: Request an Introduction
      href: /contact
    - label: Learn About Membership
      href: /membership
problem:
  heading: >-
    Traditional primary care wasn't built for people whose time is their most
    valuable asset.
  body: >-
    Even the best hospital systems in Miami, Fort Lauderdale, and Palm Beach
    are stretched thin — ten-minute visits, months-long waits for a new patient
    appointment, and a revolving door of providers who don't know your history.
    For South Florida's most accomplished individuals and families, that's not
    healthcare. That's a liability.
difference:
  heading: The Latitude 26 Difference
  items:
    - title: A Practice, Not a Panel
      body: >-
        We cap our membership at a fraction of what traditional concierge
        practices allow. That means real time — 90-minute visits, not 10 — and
        a clinician who actually knows you.
    - title: House Calls, Not Appointments
      body: >-
        Care comes to you. Home, office, hotel suite, or yacht — fully
        equipped, fully private, on your schedule. House calls are unlimited
        and included in your membership, not billed as an add-on.
    - title: Discretion by Design
      body: >-
        A pure cash-pay model means your care never touches an insurance claims
        system. No third-party data, no institutional record-keeping — just you
        and your provider.
    - title: Two Decades of Trusted Clinical Judgment
      body: >-
        Led by a physician assistant with over 20 years of frontline medical
        experience, including senior medical officer service with the U.S.
        Coast Guard and surgical care at a leading pediatric hospital —
        clinical judgment built under real pressure, now brought to your front
        door.
membership:
  heading: Membership
  intro: A single annual retainer. Everything included.
  items:
    - Unlimited house calls across Miami-Dade, Broward, and Palm Beach
    - Same-day and after-hours access, 7 days a week
    - 'Full diagnostics: labs, EKG, and point-of-care testing on site'
    - Direct line to your provider — no call center, no gatekeeping
    - Coordination with specialists when you need them
  cta:
    label: See Membership Details
    href: /membership
beyond:
  heading: Beyond the Annual Exam
  intro: >-
    For members who want more, Latitude 26 offers a curated menu of advanced
    services.
  items:
    - title: Mobile IV Wellness Infusions
      body: Recovery, hydration, and performance, delivered wherever you are.
    - title: Medical Weight Management
      body: Physician-supervised, personalized protocols.
    - title: Advanced Longevity & Diagnostic Screening
      body: >-
        Early-detection and preventive testing beyond the standard physical.
who_we_serve:
  heading: Who We Serve
  body: >-
    Latitude 26 was built for people who lead — executives, entrepreneurs,
    individuals, and families across Miami, Fort Lauderdale, and Palm Beach who
    expect the same level of excellence in their healthcare that they demand
    everywhere else in their lives.
  note: Membership is limited and by application.
  cta:
    label: Inquire About Membership
    href: /contact
closing:
  heading: Your health deserves your time — and ours.
  body: >-
    Latitude 26 is currently accepting a limited number of new members across
    Miami, Fort Lauderdale, and Palm Beach.
  cta:
    label: Schedule a Private Consultation
    href: /contact
---
